...
# === IMPORT after patch ===
from textaugment.eda import EDA
...
output_csv = "subset_sm_eda.csv"   # Output file
...
# === CUSTOM RANDOMIZED METHODS ===
def insert(text):
    n = max(1, len(text.split()) // 10)
    return augmenter.random_insertion(text, n=random.randint(1, n))

def delete(text):
    return augmenter.random_deletion(text, p=0.1)

def swap(text):
    n = max(1, len(text.split()) // 10)
    return augmenter.random_swap(text, n=random.randint(1, n))

METHODS = [insert, delete, swap]  # include full EDA for variety
